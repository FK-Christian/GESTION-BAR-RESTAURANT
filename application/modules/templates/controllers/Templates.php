...
class Templates extends CI_Controller {
    ...
    public function initialisation(){
        ...
        $this->load->view('index.php',array("titre"=>"GESTION DES TEMPLATES",'view_data'=>$this->Templates_model->get_templates()));               
    ...
    public function add_edit($id = '') {
        ...
        if ($id = $this->input->post('id')) {
            ...
            unset($data['save']);
            unset($data['id']);
            $this->Templates_model->updateRow('tb_templates', 'col_id', $id, $data);
            ...
            unset($data['fileOld']);
            unset($data['save']);
            $insert_id = $this->Templates_model->insertRow('tb_templates', $data);
    ...
    public function ajax_data() {
        $table = 'tb_templates';
        $columns = array(
            array('db' => 'col_name', 'dt' => 0),
            array('db' => 'col_id', 'dt' => 1)
        );
        $joinQuery = '';
        $primaryKey = 'col_id';
