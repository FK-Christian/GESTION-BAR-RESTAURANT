        ...
        <div class="box box-success">
          ...
          <div class="box-body">     
            <?php $compteur = 1; foreach ($espaces as $espace){?>
                <?php 
                    $test = get_data_by("*",'tb_commandes',"col_espace = $espace->col_id AND col_statut in ('O','R','F')",array());
                    $statut = (empty($test)) ? "<button class='btn-success'> <i class='fa fa-table'></i></button>" : "<button class='btn-danger'> <i class='fa fa-table'></i></button>";
                ?>
                <?php if($compteur==3){?>
                    <div class="col-xs-12" style="height: 80px;"></div>
                <?php $compteur = 1; }?>
                <div class="image-texte col-xs-6">
                    <a href="<?php echo base_url()."gestion_mobile/commandes?page_active=1&espace=".$espace->col_id;?>"><img style="height: 150px;" src="<?php echo base_url()."/assets/images/espaces/".$espace->col_photo; ?>">
                    <div class="text-on-image"><strong><?php echo $espace->col_libele; ?></strong></div>
                    <div class="text-on-image-statut"><strong><?php echo $statut; ?></strong></div></a>
                </div>
            <?php $compteur++;}?>
          </div>
          <!-- /.box-body -->
